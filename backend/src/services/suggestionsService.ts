...
export async function fetchGoogleNewsSuggestions(query: string, date: string): Promise<Suggestion[]> {
    // Janela ampla: aceita noticias dos ultimos 7 dias para nao perder conteudo
  ...
  try {
      ...
      const items = feed.items
          .filter((item: Parser.Item) => {
                    ...
                    if (!raw) return true; // incluir se nao tem data
                          const publishedAt = dayjs.utc(new Date(raw));
                    // Aceitar itens dentro da janela de 7 dias ate a data alvo
      ...
      // Se o filtro de data nao retornou nada, retornar os 20 mais recentes sem filtro
      if (items.length === 0 && feed.items.length > 0) {
              console.log('[fetchGoogleNewsSuggestions] Filtro de data sem resultados, retornando itens recentes');
...
export async function fetchNewsApiSuggestions(query: string, date: string, apiKey: string): Promise<Suggestion[]> {
    if (!apiKey) {
          console.warn('[fetchNewsApiSuggestions] NEWSAPI_KEY nao configurada');
          return [];
    }
...
export async function getCachedSuggestions(
    ...
    // Cache valido por 6 horas (nao apenas no dia exato)
